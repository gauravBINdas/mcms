...
<div id="chPwd" class="container">
 <form id="pwdChange" action="changeuserpwd.php" method="post">
...
if(isset($_POST['pwdSubmit'])){
  ...
  { 
	  ...
	  echo "<script type='text/javascript'>alert('$message');window.location='/mcms/changeuserpwd.php';</script>";
  }
  else{
  $sql1="select password from users where email_id='$email'";
  $result1=mysqli_query($conn,$sql1);
  $row = mysqli_fetch_assoc($result1);
  if($row["password"]==$curr_pwd)
  {
  $sql = "update users set password='$cnf_pwd' where email_id='$email'";
